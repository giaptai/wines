                    ...
                    <div class="p-3 row row-cols-1 row-cols-md-3 sticky-top bg-light">
                        <div class="col-md-auto">
                            ...
                            <label class="btn btn-outline-primary btn-sm" for="btnradio1">Tổng sản phẩm <span
                                    class="badge bg-danger" id="badge_tongdon"> <?php echo sizeof($wineArray); ?></span></label>
                        </div>
...
<script>
    ...
    function edit(id) {
        ...
        xhttp.onreadystatechange = function() {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                ele.children[4].innerHTML=new Intl.NumberFormat('de-EN', { style: 'currency', currency: 'VND' }).format(s7);
                ele.children[5].innerHTML=new Intl.NumberFormat('de-EN', { style: 'currency', currency: 'VND' }).format(s6*s7);
    ...
    function searched(val) {
        ...
        //cau hinh request
        xhttp.open('GET', "./products/searched?search_id="+arr[0]+"&search_name="+arr[1]+"&page="+arr[2], true);
        //cau hinh header cho request
        xhttp.setRequestHeader('X-CSRF-TOKEN', document.querySelector('meta[name="csrf-token"]').content);
        xhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhttp.send();
